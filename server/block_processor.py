...
class WitnessEncoder(json.JSONEncoder):  
    def default(self, obj):  
        if isinstance(obj, Witness):  
            return obj.__dict__  
        return json.JSONEncoder.default(self, obj)
        
class WitnessScanner(LoggedClass):
    '''Scan witness info to local cache.'''
    ...
    async def main_loop(self):
        '''Loop forever polling for witness.'''
        while True:
            await asyncio.sleep(5)
            with await self.semaphore:
                self.witness = await self.daemon.getallwitnesses()
                #if not self.witness :
                    #self.witness.append(Witness(1, "aaa", "12p1StPMu7STbpfyy81rGAd5xwhC82MWej"))

    async def get_witness(self, name):
        '''Get witness info according name.'''
        with await self.semaphore:
            if not name :
                return self.witness
            else :
                #ret_val = Witness()
                ret_val = {'id':0, 'name':'', 'address':'' }
